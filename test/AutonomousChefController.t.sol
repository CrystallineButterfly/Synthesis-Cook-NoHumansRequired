...
import {AutonomousChefController} from "src/AutonomousChefController.sol";

contract AutonomousChefControllerTest is Test {
    AutonomousChefController private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_cook_nohumansrequired");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new AutonomousChefController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        AutonomousChefController freshController = new AutonomousChefController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
