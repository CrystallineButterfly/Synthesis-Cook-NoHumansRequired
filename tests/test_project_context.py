...
MODULE = importlib.import_module('agents.autonomous_chef')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'autonomous_chef')
        self.assertEqual(spec.primary_contract_name, 'AutonomousChefController')
